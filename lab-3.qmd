...
a.  `glimpse()` at the `country_inflation` data frame and answer the following questions based on the output. How many rows does `country_inflation` have and what does each row represent? How many columns does `country_inflation` have and what does each column represent?
    ...
    It has 44 rows that represent an observation of inflation in a country during a specific year and 30 columns that represent the variables (country, year, inflation).
...
The countries that had the top 3 inflation rates were Argentina, Turkey, and Brazil. They all had significantly higher inflation rates than the United States rate at 5%, with Argentina at 48%, Turkey at 20%, and Brazil at 8%.
...
New Zealand had the largest inflation between the years 1993 and 2021, with the inflation being positive.
...
```{r}
inflation_new <- country_inflation |>
  pivot_longer(
    cols = -country,
    names_to = "year",
...
glimpse(inflation_new)
```
:::

The resulting dataset has 3 columns and 1276 rows.
...
a.  What is the highest inflation rate observed between 1993 and 2021? The output of the pipeline should be a data frame with one row and three columns. In addition to code and output, your response should include a single sentence stating the country and year.
    ...
    inflation_new |>
     filter(annual_inflation == max(annual_inflation, na.rm = TRUE)) |>
      select(country, year, annual_inflation)
    ```

    Brazil had the highest inflation rate in 1994 at 2075.888%.
...
b.  What is the lowest inflation rate observed between 1993 and 2021? The output of the pipeline should be a data frame with one row and three columns. In addition to code and output, your response should include a single sentence stating the country and year.
    ...
    Ireland had the lowest inflation rate in 2009 at -4.478103%.
...
countries_of_interest <- c(
  "Canada", "United States", "China (People's Republic of)"
)
```
:::

I chose the 3 largest countries within the dataset to see how their inflation rates compare.
...
```{r}
inflation_coi <- inflation_new |>
filter(country %in% countries_of_interest)
glimpse(inflation_coi)
```
...
```{r}
ggplot(inflation_coi, aes(x = year, y = annual_inflation, 
                          color = country, shape = country, alpha = 0.05)) +
geom_line(size = 1) +
geom_point(size = 2) +
scale_color_manual(
  values = c(
   "Canada" = "Red",
   "United States" = "Blue",
   "China (People's Republic of)" = "Yellow"
  )
) +
labs(
  title = "Annual Inflation Trends of Some of the Largest Countries",
  subtitle = "1993-2021",
  x = "Year",
  y = "Annual Inflation (%)",
  color = "Country",
...
China seemed to have much higher levels of inflation during the earlier years, roughly from 1993-1997, in comparison to both the United States and Canada which is quite surprising. All 3 countries were negatively effected by the 2008 market crash but rebounded in the few years after. Lastly, the United States as well as Canada seem to have higher rates of inflation during recent years in comparison to China.
...
The dataset has 132 rows and 4 columns. The 4 different variables include country which only includes the United States, cpi_division_id which includes the 12 different divisions, year which tells the year, and lastly the annual inflation.
...
The cpi_divisions dataset has 12 rows and 2 columns. The variables include id number and descriptions associated with each number.
...
```{r}
us_inflation_new <- us_inflation |>
  left_join(cpi_divisions, by = c("cpi_division_id" = "id"))
glimpse(us_inflation_new)
```

The dataset has 132 rows and 5 columns. The column names include country, cpi_division_id, year, annual_inflation, and lastly description.
...
I chose these IDs specifically as they are necessities for most if not all people.
...
```{r}
us_inflation_divisions <- us_inflation_new |>
  filter(description %in% divisions_of_interest)
```

```{r}
us_inflation_divisions |>
...
```{r}
us_inflation_divisions |>
ggplot(aes(x = year, y = annual_inflation, colour = description, 
           shape = description, alpha = 0.05)
) +
  geom_line() +
  geom_point(size = 2) +
  scale_colour_manual(
    values = c(
      "Housing, water, electricity, gas and other fuels" = "black",	
      "Health" = "red",
      "Transport" = "blue",	
      "Education" = "yellow"
    )
  ...
  labs(
    title = "U.S Annual Inflation vs CPI Division",
    subtitle = "2011-2021",
    x = "Year",
    y = "Annual Inflation (%)",
    colour = "Divisions",
...
Transport has seemed to experience the most variation in its rate of inflation over the years when compared to the other 3 divisions displayed within the plot. I would argue with my very little economice knowledge that transportation is less significant to people in comparison to the other 3 divisions, thus it's variation in demand leads to greater variation of inflation. Its quite surprising how the other 3 divisions stayed so relatively similar in terms of inflation over the years.
